    ...
    public partial class WoForm : Form
    {
        ...
        private void buttonCreateWorkOrder_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(textBoxInspectionId.Text))
            {
                ...
                create.WOTemplateId = ((WOTemplateName)comboBoxWoTemplateId.SelectedItem).WOTemplateId; // ((KeyValuePair<string, string>)comboBoxWoTemplateId.SelectedItem).Key;
                create.EntityType = ((PWEntity)comboBoxWoEntityType.SelectedItem).Code; // ((KeyValuePair<string, string>)comboBoxWoEntityType.SelectedItem).Key;
                create.SubmitTo = ((EmployeeBase)comboBoxSubmitTo.SelectedItem).EmployeeSid; // ((KeyValuePair<int, string>)comboBoxSubmitTo.SelectedItem).Key;
            ...
            else
            {
                ...
                // Set the properties of the create object to the contents of the various inputs.
                woCreation.WOTemplateId = ((WOTemplateName)comboBoxWoTemplateId.SelectedItem).WOTemplateId; // ((KeyValuePair<string, string>)comboBoxWoTemplateId.SelectedItem).Key;
                woCreation.EntityType = ((PWEntity)comboBoxWoEntityType.SelectedItem).Code; // ((KeyValuePair<string, string>)comboBoxWoEntityType.SelectedItem).Key;
                woCreation.SubmitTo = ((EmployeeBase)comboBoxSubmitTo.SelectedItem).EmployeeSid; // ((KeyValuePair<int, string>)comboBoxSubmitTo.SelectedItem).Key;
        ...
        public void SetupComboBoxes()
        {
            ...
            var domResponse = _services.General.Authentication(new AuthenticationService.Domains());
            // Check the value of the response. If the response is bad, alert the user.
            if (domResponse.Status != 0) ErrorMessage();
            
            // Next, get entity groups using the domain ID and the entity groups API.
            EntityService.GroupsResponse groupsResponse = _services.AMS.Entity(new EntityService.Groups { DomainId = domResponse.Value[0].DomainId });
            // Check the value of the response. If the response if bad, alert the user.
            if (groupsResponse.Status != 0) ErrorMessage();
            // Declare a list and add all entity group modules.
            List<string> entityGroups = new List<string>();
            foreach (var group in groupsResponse.Value) { entityGroups.Add(group.Module); }
            ...
            EntityService.Types types = new EntityService.Types { EntityGroups = entityGroups };
            // Make the API call and save the response to a variable.
            var typesResponse = _services.AMS.Entity(types);
            // Check the value of the response. If the response is bad, alert the user.
            if (typesResponse.Status != 0) ErrorMessage();
            // Populate the entity type combo box.
            comboBoxWoEntityType.DataSource = typesResponse.Value;
            comboBoxWoEntityType.ValueMember = "Code";
            comboBoxWoEntityType.DisplayMember = "Description";
        ...
        private PWEntity GetSelectedEntityType()
        {
            ...
            return ((PWEntity)comboBoxWoEntityType.SelectedItem);
        }
        ...
        private void LoadWorkOrderTemplateList(PWEntity selEntity)
        {
            ...
            WorkOrderTemplateService.TemplateNames templateNames = new WorkOrderTemplateService.TemplateNames();
            // Declare a list to hold the code of the selected entity type, and store that code.
            List<string> entityCode = new List<string>();
            entityCode.Add(selEntity.Code);
            // Set the entity types property of the work order template object.
            templateNames.EntityTypes = entityCode;
            // Make the API call and save the response to a variable.
            WorkOrderTemplateService.TemplateNamesResponse templatesResponse = _services.AMS.WorkOrderTemplate(templateNames);
            // Populate the combo box with the response.
            comboBoxWoTemplateId.DataSource = templatesResponse.Value;
            comboBoxWoTemplateId.ValueMember = "WOTemplateId";
            comboBoxWoTemplateId.DisplayMember = "Description";
        ...
        private void LoadEmployees()
        {
            ...
            EmployeeService.All all = new EmployeeService.All { IncludeInactive = false };
            // Make the API call and save the response to a variable.
            var response = _services.AMS.Employee(all);
            // Populate the submit to combo box with the employees returned.
            comboBoxSubmitTo.DataSource = response.Value;
            comboBoxSubmitTo.ValueMember = "EmployeeSid";
            comboBoxSubmitTo.DisplayMember = "FullName";
        }
        #endregion
