...
# Function to get inspection templates using the selected entity type.
def insp_template_templates():
    """Function to getr inspection templates using the selected entity type."""
    data = {'EntityTypes':[entity_type]}
    parameters = data_to_json(data)
    url = base_url + 'ams/inspectiontemplate/templates'
    response = make_request(url, parameters)
    r_value = ''
    if response['Status'] == 0:
...
templates = insp_template_templates()
if templates == '':
